...
class HomeController < ApplicationController
  ...
  def index
    ...
    @count_ddjjs_3 = Rails.cache.fetch("count_ddjjs_3", :expires_in => CANT_DJ_Y_ULT_ACT) do
                  q = Ddjj.includes({:persona_cargo=>:cargo}).where("cargos.poder_id = ?", 3).count
            end
  end
